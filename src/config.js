...
(function (LB) {
  ...
  LB.DEFAULT_CONFIG = {
    ...
    // 危ないマス（決着をつきやすくするための仕組み。仕様書 §35）
    //   DANGER_ZONE          … true で有効（対戦設定の「危ないマス」）
    //   DANGER_ZONE_START    … この手数を過ぎると、外周1周が危ないマスになる（両者合わせた手数）
    //   DANGER_ZONE_INTERVAL … 危ないマスになってから、自分の手番 N 回ごとに、
    //                          危ないマスにいる自分の騎が DANGER_ZONE_DAMAGE を受ける
    DANGER_ZONE: false,
    DANGER_ZONE_START: 60,
    DANGER_ZONE_INTERVAL: 3,
    DANGER_ZONE_DAMAGE: 1,

    // 考える時間（秒）。0 で無制限。時間切れになると自動でランダムに1手指す
    THINK_TIME: 20,
